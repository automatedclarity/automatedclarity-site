...
export default async (req) => {
  ...
  if (!password || password !== expected) {
    if ((req.headers.get("content-type") || "").includes("application/x-www-form-urlencoded")) {
      return new Response(`<!doctype html><meta charset="utf-8"><script>location.href="/matrix-login?e=bad"</script>`, {
  ...
  if ((req.headers.get("content-type") || "").includes("application/x-www-form-urlencoded")) {
    return new Response("", { status: 303, headers: { "Set-Cookie": cookie, "Location": "/matrix", "Cache-Control":"no-store" } });
  }
