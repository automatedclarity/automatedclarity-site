...
export default async (req) => {
  ...
  if (!locationId) {
    return json(400, {
      ok: false,
      error: "Missing location_id in payload",
      received: body,
    });
  }
  ...
  const customField = [
    ...
    // Started At (string) — NOTE: key includes "console_"
    setCF("acx_console_started_at_str", startedAtStr),
  ].filter(Boolean);

  // Hard-fail if started_at_str can't be found (no silent "works")
  if (!keyToId.get("acx_console_started_at_str")) {
    return json(422, {
      ok: false,
      error:
        "Field key not found in location customFields (case-insensitive lookup failed).",
      missing_key: "acx_console_started_at_str",
      location_id: locationId,
  ...
  return json(200, {
    ...
    has_console_started_at_str_id: Boolean(
      keyToId.get("acx_console_started_at_str")
    ),
    console_started_at_str_value: startedAtStr,
  });
};
